...
**The Fix:** We built a dynamic data intake system. An async crawler (Playwright) runs in the background to autonomously scrape Google Cloud's customer success website and batch them to [Google Cloud Vector Search](https://cloud.google.com/vertex-ai/docs/vector-search/overview). Back in the pipeline, the Case Study Researcher runs a true Hybrid Search on the indexed corpus to fetch ideal case studies. *(Note: Hybrid Search combines the semantic "meaning" of a query with the precision of exact keyword matching, ensuring the agent doesn't miss specific technical terms).*
